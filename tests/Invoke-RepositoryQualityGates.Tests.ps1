...
'@
    ...
    Assert-True (($versionOutput -join "`n").Contains('Repository Quality Gates 1.5.8')) 'The version interface should report the canonical version.'
    ...
    Assert-True (($embeddedVersionOutput -join "`n").Contains('Repository Quality Gates 1.5.8')) 'The embedded deployment engine should match the canonical release version.'
    Assert-True ((Get-Content -LiteralPath (Join-Path $projectRoot '.repository-quality-gates.json') -Raw | ConvertFrom-Json).templateVersion -eq '1.5.8') 'The central managed state should match the canonical release version.'
