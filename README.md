...
The prepared template version written to managed state is `1.5.6`. The automatic release workflow publishes the matching immutable `v1.5.6` GitHub Release after the required checks pass for the release commit.

Version `1.5.6` reports the actual repository-specific self-hosted runner names from downstream workflow jobs and preserves sanitized installation-stage failure details for decisive investigation. The email retains the status-count summary and fully bordered Repository, Visibility, Runner, Status, and Comment table introduced in `1.5.5`, including the approved sender and recipient names. Repository discovery uses the GitHub App installation API directly, retaining each repository's visibility and organization repositories in the structured report. Comments remain short factual summaries; complete sanitized diagnostics remain in the private workspace report used to produce the email. The downloadable one-day diagnostic artifact remains separately sanitized and excludes repository identities. Public repositories and pull requests from forks continue to use GitHub-hosted runners. See [Automatic Repository Updates](docs/automatic-repository-updates.md) for the security model and one-time setup.
