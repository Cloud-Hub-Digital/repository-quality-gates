...
The prepared template version written to managed state is `1.5.8`. The automatic release workflow publishes the matching immutable `v1.5.8` GitHub Release after the required checks pass for the release commit.

Version `1.5.8` registers every discovered workflow runner name for masking before public rollout output is retained. The private named email keeps the actual repository-specific runner names, while the downloadable one-day diagnostic artifact excludes repository and runner identities. It retains the `1.5.7` per-installation authentication refresh, status-count summary, and fully bordered Repository, Visibility, Runner, Status, and Comment table. Comments remain short factual summaries; complete sanitized diagnostics remain in the private workspace report used to produce the email. Public repositories and pull requests from forks continue to use GitHub-hosted runners. See [Automatic Repository Updates](docs/automatic-repository-updates.md) for the security model and one-time setup.
