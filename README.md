...
The prepared template version written to managed state is `1.5.5`. The automatic release workflow publishes the matching immutable `v1.5.5` GitHub Release after the required checks pass for the release commit.

Version `1.5.5` adds a status-count summary and a fully bordered Repository, Visibility, Runner, Status, and Comment table to each fleet rollout email. It uses the actual Actions runner name and displays the approved sender and recipient names. Repository discovery now uses the GitHub App installation API directly, retaining each repository's visibility and restoring organization repositories to the structured report. Comments remain short factual summaries; complete sanitized diagnostics remain in the private workspace report used to produce the email. The downloadable one-day diagnostic artifact remains separately sanitized and excludes repository identities. Public repositories and pull requests from forks continue to use GitHub-hosted runners. See [Automatic Repository Updates](docs/automatic-repository-updates.md) for the security model and one-time setup.
