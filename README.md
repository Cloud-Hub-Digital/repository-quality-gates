...
The prepared template version written to managed state is `1.5.9`. The automatic release workflow publishes the matching immutable `v1.5.9` GitHub Release after the required checks pass for the release commit.

Version `1.5.9` reconciles exact GitHub-hosted runner selectors in repository-owned workflows. Trusted events in private repositories use the configured repository-specific or organization-shared self-hosted runners, while public repositories and pull requests from forks retain each workflow's original GitHub-hosted fallback. It retains the `1.5.8` runner-name masking, private named email, one-day sanitized diagnostic artifact, status summary, and fully bordered Repository, Visibility, Runner, Status, and Comment table. See [Automatic Repository Updates](docs/automatic-repository-updates.md) for the security model and one-time setup.
