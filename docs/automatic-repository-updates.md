...
The rollout job has a 120-minute overall limit. Each downstream pull request may wait up to 45 minutes for reported checks to finish, after allowing up to four minutes for the first check to appear. When email reporting is enabled, the workflow sends a final success or failure report after the rollout step, including the selected release, workflow-run link, and captured per-repository output. A rollout failure remains a workflow failure after the report is sent.
